...
Status DBImpl::Get(const ReadOptions& options,
  ...
  Status s;
  MutexLock l(&mutex_);
  SequenceNumber snapshot;  // 此次GET的sequence number
  ...
  MemTable* mem = mem_;
  MemTable* imm = imm_;
  Version* current = versions_->current();  // 为什么这里是当前版本
  mem->Ref();
...
Status DBImpl::MakeRoomForWrite(bool force) {
  ...
  while (true) {            // 这个循环会一直执行，直到memtable有足够的空间，或者后台线程出错
    if (!bg_error_.ok()) {  // 后台线程执行有错误，直接返回
      ...
      // 重新分配一个MemTable用于更新操作，memtable和log应该是同步的，同时切换
      assert(versions_->PrevLogNumber() == 0);
      uint64_t new_log_number = versions_->NewFileNumber();
      WritableFile* lfile = nullptr;
      s = env_->NewWritableFile(LogFileName(dbname_, new_log_number), &lfile);  // 根据文件名open一个log文件
      if (!s.ok()) {
